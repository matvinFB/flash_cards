...
void startGesture(DateTime startTime, Offset offset) => currentGesture = Gesture(startPoint: Point(offset.dx, offset.dy), start: startTime);

void addPointToGesture(Offset offset) => currentGesture?.addPoint(Point(offset.dx, offset.dy));
...
print("=========Gesture=========");
  ...
  int timeMs = currentGesture!.end.difference(currentGesture!.start).inMilliseconds; 
  print("Total duration: ${currentGesture!.end.difference(currentGesture!.start).inMilliseconds} ms");
  Point temp = calcFinalVector(currentGesture!.trajectory.first, currentGesture!.trajectory.last,);
  print("Final vector (x, y): (${temp.x}, ${temp.y})");
  print("Final vector size: " + calculateDistance(currentGesture!.trajectory.first, currentGesture!.trajectory.last,).toString() );
  double totalDist = calculateTotalDistance(currentGesture!.trajectory);
  print("Total Gesture Size: $totalDist");
  print("Média de Pixels por milissegundos: ${(totalDist/timeMs).toStringAsFixed(3)}");
  print("====================\n");
